...
function t2() {
    ...
    let two = a2.map(function (item) {
        return a2_res.push(item * item);
    });
...
function t4() {
    ...
    let four = a4.forEach(function (item) {
...
function t5() {
    let b1 = [3, 14, 15, 92];
    let five = b1.filter(function (item) {
        if (item % 2 == 0) {
            b1_res.push(item);
        }
    })
...
function t8() {
    ...
    let eight = b8.filter((item,index) => {
...
function t9() {
    ...
    document.querySelector(".out-9").innerHTML = `number: ${b9_num}<br>string: ${b9_string}`;
}
...
function t10() {
    ...
    let count = "";
    let ten = b10.filter((item, index, array) => {
        item.filter(element => {
            if (element == 3) {
                b10_res.push(item)
            }
